/**
 * @file
 * Example 028: Creating a permission profile
 * @author DocuSign
 */
...
const eg028CreatePermission = exports
 ...
 */
eg028CreatePermission.createController = async (req, res) => {
    ...
    try {
        results = await accountsApi.createPermissionProfile(args.accountId, {
            permissionProfile: {
 ...
 */
eg028CreatePermission.getController = async (req, res) => {
    // Check that the authentication token is ok with a long buffer time.
    // If needed, now is the best time to ask the user to authenticate
    // since they have not yet entered any information into the form.
    let tokenOK = req.dsAuthCodeGrant.checkToken();
    if (tokenOK) {
        res.render('pages/examples/eg028CreatePermission', {
